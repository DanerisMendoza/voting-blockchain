...
class CandidateController extends Controller
{
    ...
    public function GetAllCandidates()
    {
        $userDetail = DB::table('candidates')
            ...
            ->join('positions', 'positions.id', 'candidates.position_id')
            ->join('partylists', 'partylists.id', 'candidates.partylist_id')
            ->select(DB::raw("CONCAT(users.first_name, ' ', users.middle_name, ' ', users.last_name) as candidate_name"), 'positions.name as position_name', 'candidates.id as candidate_id', 'positions.id as position_id', 'partylists.name as party_list', 'users.profile_pic_path', 'users.gender')
            ->get()
            ->each(function ($q) {
                if ($q->profile_pic_path != null) {
                    $image_type = substr($q->profile_pic_path, -3);
